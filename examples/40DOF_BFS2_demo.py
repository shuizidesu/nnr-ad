...
dx[:, 0:1] = dx0
ddx[:, 0:1] = ddx0
x1 = torch.zeros([40, 1], device=device, dtype=torch.double)
...
for step in range(1, solve_steps+1):
    ...
    while not ((epoch>max_epoch) or (incremental<incremental_tolerance) or (residual<residual_tolerance)):
        residual_equation_k = numerical_solver.calculate_residual(x1, x0, dx0, ddx0, current_time)
        delta_x1 = -jacobian_inv @ residual_equation_k
        x1 = x1 + delta_x1

        residual_equation_kk = numerical_solver.calculate_residual(x1, x0, dx0, ddx0, current_time)
        yk = residual_equation_kk - residual_equation_k

        muk = (1.0 + yk.T @ jacobian_inv @ yk / (delta_x1.T @ yk)).squeeze()
        jacobian_inv = jacobian_inv + (muk * delta_x1 @ delta_x1.T - delta_x1 @ yk.T @ jacobian_inv -
                                       jacobian_inv @ yk @ delta_x1.T) / (delta_x1.T @ yk)

        incremental = torch.norm(delta_x1)
        residual = torch.norm(residual_equation_kk)
        epoch += 1
